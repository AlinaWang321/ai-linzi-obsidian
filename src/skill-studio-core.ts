...
任一步失败就停在该步，不把后续动作说成已完成。`,
  ...
  makeTemplate({
    ...
    permissions: [
      ...
      '本机只保存文件指纹和 AI 提炼的结构化摘要，不保存原始正文；中断后可从断点继续',
      '结合 AI霖子当前任务清单整理今天待办',
...
- 首次运行把原文分成有界批次，每批完成后立即在本机 IndexedDB 保存结构化摘要与 path/mtime/size 指纹，不保存原始正文。即使最终 HTML 暂未生成，已完成摘要也保留；再次运行会从未完成文件继续。以后刷新只重读新增或改动正文，未变文件直接复用本机摘要，不把旧正文重复发送给 AI。

## 工作步骤
1. 插件先建立最近 7 天固定文件清单，再由经营周报专用管道自动完成 Map → Reduce：每批只上传有界原文，收到摘要后立即丢弃原文上下文并把摘要保存在本机；扫描必须持续到所有文件进入“已摘要”或“已跳过”，不能因单批 complete=false 停止。
   - 首次运行：逐批处理所有可读文件，但从不把前面原文重复带进下一批。
   - 中断续跑：复用已经落地的分段摘要，从尚未完成的文件/分段继续。
   - 后续刷新：按 path/mtime/size 只重新处理新增或改动文件，未变文件直接复用本机摘要；被删除或移出七天窗口的摘要自动移出本次合并。
2. 默认排除用户设置的整个 AI霖子输出目录、我的 Skills 目录和插件保护文件，避免把 AI 自己的产物再次总结成本周经营动作。图片、音视频等未提取附件进入“跳过”清单，不冒充已读正文。
3. 看板必须显示管道返回的统计口径提醒：最近 7 天按文件修改时间计算，同步、git pull 或批量脚本改写可能影响口径。如果仍有未摘要、未跳过的文件，停止生成并保留断点，不输出伪完整周报。
