...
- 当你要求整理、移动、重命名或删除笔记时，AI 只先核实真实路径并给方案；你在 Obsidian 再次确认后插件才执行。删除功能一次只接受一篇 Markdown 笔记，只移入系统废纸篓/回收站（系统不允许时由 Obsidian 放入 `.trash`），不能删除附件、文件夹、批量内容，也不会永久删除。移动/重命名可用命令面板「撤销上一次 AI Vault 整理」恢复；回收站笔记请从回收站恢复。
