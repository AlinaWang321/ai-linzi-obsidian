...
- 开启「智能搜索 Vault」后，插件会在你发送问题时临时在本机搜索 Vault 内的普通文档，只把少量匹配片段交给 AI 回答；不会上传整个 Vault，也不会建立云端全文索引。
- 开启后，AI 还可以分步请求“继续搜索、查看目录、读取某篇文档的一段”，但所有读取都由插件在你的电脑上校验执行，AI 没有直接磁盘权限。工具过程和本地路径不会进入云端聊天历史。
- 当你要求整理、移动或重命名时，AI 只先给方案；你在 Obsidian 二次确认后插件才执行。当前版本不删除文件、不覆盖同名文件，移动/重命名可用命令面板「撤销上一次 AI Vault 整理」恢复。
- 当前笔记、主动选择的整篇文档、图片修改和公众号发布，仍然只有在你主动勾选或确认后才会发送或执行。
