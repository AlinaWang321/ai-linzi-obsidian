...
const globalStyles = await readFile(new URL('../styles.css', import.meta.url), 'utf8')
...
assert.match(globalStyles, /\.ai-linzi-consultation-body li\.task-list-item\s*\{[^}]*display:\s*block;/)
assert.doesNotMatch(globalStyles, /\.ai-linzi-consultation-body li\.task-list-item\s*\{[^}]*display:\s*flex;/)
assert.match(globalStyles, /overflow-wrap:\s*anywhere;/)
assert.match(globalStyles, /li\.task-list-item input\[type="checkbox"\]\s*\{[^}]*position:\s*absolute;/)
...
const taskItem = new FakeElement()
const taskCheckbox = new FakeElement()
...
body
  ...
  .add('th, td', th, td)
  .add('th', th)
  .add('li.task-list-item', taskItem)
  .add('li.task-list-item input[type="checkbox"]', taskCheckbox)
...
assert.equal(taskItem.style.get('display'), 'block')
assert.equal(taskItem.style.get('overflow-wrap'), 'anywhere')
assert.equal(taskCheckbox.style.get('position'), 'absolute')
assert.equal(taskCheckbox.style.get('left'), '14px')
