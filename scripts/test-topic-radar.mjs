...
assert.match(actions, /readNoteSnapshot\(plugin, activeFile\)/)
assert.doesNotMatch(actions, /input\.useCurrentNote \? await getActiveNote\(plugin\)/)
...
assert.match(main, /const explicitVaultRequest = shouldUseVaultAgent\(text, false\)/)
assert.match(main, /shouldUseVaultAgent\(text, true\)/)
