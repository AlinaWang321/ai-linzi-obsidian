...
console.log('[test-skill-studio-ui] 自建 Skill 的名称、触发短语和测试示例共用实时自检')
{
  ...
  modal.onOpen()
  assert.equal(setting('读取范围').controls.length, 0)
  assert.match(setting('读取范围').desc, /整个 Vault/)
  assert.equal(
...
console.log('[test-skill-studio-ui] 外部 Skill 导入默认整个 Vault，不再让新手选安装时文件夹')
{
  globalThis.__ui.settings = []
  const offered = []
  const block = {
  ...
  modal.onOpen()
  assert.equal(setting('读取范围').controls.length, 0)
  assert.match(setting('读取范围').desc, /整个 Obsidian Vault/)
  ...
  assert.equal(manifest.vaultRead.fixedFolder, undefined)
