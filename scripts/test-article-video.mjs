...
assert.match(runtime, /scriptModal\.open\(\)/)
assert.match(runtime, /确认脚本并生成视频/)
assert.match(runtime, /确认脚本、安装环境并生成/)
assert.match(runtime, /最终成片不强行拉伸到目标时长/)
assert.doesNotMatch(runtime, /\.setName\('视频时长'\)/)
assert.ok(
  runtime.indexOf("apiText('/api/plugin/v1/skills/article-to-video'") < runtime.indexOf('scriptModal.open()'),
  '必须先生成脚本草稿，再让用户修改并确认',
)
