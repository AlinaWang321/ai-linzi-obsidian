...
- **v0.6.19**：新增“我的工作流”MVP。学员可以在“调用技能”里创建、运行、编辑和删除自己的固定流程，也可以把刚刚说过的任务要求一键保存为工作流；工作流以 Markdown 留在自己的 Vault，运行前确认本轮当前笔记与已选文件，仍统一走插件专用 API。
...
- 在「调用技能 → 我的工作流」中创建固定流程，例如“批量阅读咨询逐字稿并整理客户洞察”。
- 运行前会显示本轮是否带上当前笔记、已经选择了多少篇文件；用户点击「开始运行」后才会发送。
- 每条普通 AI 回复底部都有「保存为工作流」；如果学员先让 AI霖子设计一套工作流，按钮会变成「保存这套工作流」，把 AI 已经整理好的流程带入编辑窗口，确认后保存。
- 工作流文件默认保存在 Vault 根目录的 `AI霖子工作流/`，可以在插件设置中改路径，也可以随 Vault 备份或同步。
- 创建、浏览、编辑和删除都在本地完成，不调用模型；真正运行时才进入正常主对话链路。多文件授权仍是 Pro/Business 权益，不能通过工作流绕过。
