...
- **v0.6.5**：新增模型无关的「用 AI 生图」入口，支持可选参考图；文章配图修改先预览、确认后才替换原图
...
- **v0.6.1**：插件对话历史单向同步到云端；网页版可查看插件对话，插件只显示和清空自己产生的对话
...
对话面板提供「用 AI 生图」：可直接描述画面，也可从 Vault 或电脑添加最多 3 张参考图。生成结果先在对话框预览，用户确认后再保存或插入笔记。文章配图生成后可以选择单图修改；修改版先与原图并排预览，只有用户点击确认才会备份并替换原图。
...
- 插件历史只读取和删除 `obsidian:` 命名空间；插件对话可以在网页版查看，但网页端和微信端对话不会同步回插件，也不会被插件的清空操作删除
