...
- **v0.6.21**：主对话默认在用户电脑本地智能搜索整个 Vault，只把最多 6 条相关短片段交给 AI，不调用向量 API、不上传整个 Vault；名称含「㊙️」、隐藏目录、废纸篓和用户设置的排除文件夹不会进入搜索。回复下方显示可点击的来源笔记；原 Pro/Business 精确文件/文件夹选择保留为输入框旁的附件按钮。
