...
- 第一张配图完整显示且不加图片外框；其余 Markdown、Wiki 或 HTML 图片按原文顺序进入正文卡片。正文图片会适当缩小，短上下文采用左图右文；分页器避免章节标题落单，并把过稀的末页内容向前回填，尽量填满每一页。第一页已经显示的正文会从后续分页中扣除，Markdown 金句加粗继续保留。
