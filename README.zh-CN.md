...
- **v0.7.89**：修复 Skill Studio 创建新 Skill 被误判为修改已有 Skill；官方「咨询交付闭环」已有逐字稿、规则和目录结果时，不再错误声称没有 Vault、CRM、任务或 PNG 能力。客户咨询简报生成后可直接在主对话说“把刚才的咨询简报里……改成……”，插件使用只存本机会话的最新版隐藏源稿重新生成一张 PNG，不在 Vault 多建 Markdown、不覆盖旧图；修改结果若严重缩水、丢失未点名章节或没有落实修改会失败关闭。
- **v0.7.88**：Skill Studio、自建 Skill 与外部 Skill 导入统一默认按需读取整个 Vault，移除安装时的文件夹权限选择；用户在运行时点名文件或文件夹，只是缩小当次搜索范围。主对话同时要求“封装成 Skill 并完成这次任务”时，确认按钮会创建 Skill 后立即续跑原任务。模型漏写 manifest v2 字段、可见权限或已有 reference 的入口链接时，本机会按确定规则补齐再校验，不再要求用户反复重新生成。官方「咨询交付闭环」的第 2–5 步始终沿用第 1 步锁定的 TXT/PDF/DOCX/Markdown 原材料，不再要求把 TXT 打开成当前 Markdown 笔记。
...
- **v0.7.73 历史实现记录**：该版曾引入 Skill 安全更新、完整差异/删除预览、确认前全树 SHA-256 复核和失败回滚。其中插件自建历史版本的设计已在后续版本中废止；旧 Vault 如已存在 `ai-linzi-versions`，新版不读写也不自动删除。
...
- **v0.7.64**：① **Word 里的图片会进课件了**。用 .docx 讲义生成课件PPT 时，文档内嵌的截图/配图会按它在原文出现的位置被提取出来，排进讲同一件事的那一页（图片只在本机解析压缩，原文件不上传）。同时收紧了配图规则：给了 N 张图，成品里的图片页不少于 min(N, 一半页数)——不再出现「给了十几张图却只用两张」。② **交互看板换成品牌视觉**。主对话生成的 HTML 看板和「知识库日报看板」等技能产出，从旧的橙蓝配色改为「水·木·光」品牌设计：暖米底＋海军蓝结构＋大留白，新增顶部判断条（宋体金句）、指标卡（数字滚动）、进度条走水→木→光渐变，「盲区/风险」类卡片用陶土色单独标出，深色模式跟随品牌墨蓝。③ 点击「我的 Skills」里的技能时，填入的话改为该技能自己声明的触发语——处理整个知识库的技能不会再被写成「处理当前笔记」。
- **v0.7.63**：新增内置技能「课件PPT」。在「调用技能」里选择一份 MD/TXT/PDF/DOCX 讲义，AI 编排分页大纲后，插件按固定品牌模板在本机装配成可全屏放映的网页课件：三套主题色（深蓝/青竹/黛紫）、文档里的图片自动压缩内嵌、页脚署名用你自己的名字；用系统浏览器打开后按 F 全屏放映、⌘P（Windows Ctrl+P）直接存成 16:9 PDF。文档与图片都在本机解析，原始文件与图片不上传。另修复：从访达/资源管理器把**已在库内**的 PDF/Word 拖进对话框时被误判「不在知识库里」（现在会正确识别为库内资料，含中文文件名规范化）。
...
npm run check:marketplace # Obsidian 官方 lint 阻断项 + 市场包安全检查
```
...
- 当前正式版已进入 Obsidian 官方社区插件目录；首次安装优先在「设置 → 第三方插件 → 浏览」搜索 **AI Linzi**，安装并启用
- 版本更新统一通过 Obsidian「第三方插件 → 检查更新」完成，不使用插件内自更新器
- 只有开发验收或官方目录临时不可用时，才从 AI霖子网页版「连接中心」下载手动测试包；安装说明同时打进安装包
- 手动安装时，把解压后的 `ai-linzi` 文件夹放进 Vault 的 `.obsidian/plugins/`
...
- 手动测试包用户后续可切回官方市场更新；覆盖 `main.js`、`manifest.json`、`styles.css` 不会覆盖笔记或账号云端对话
