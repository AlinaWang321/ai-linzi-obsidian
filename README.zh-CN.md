...
- **v0.7.67 候选版（尚未上架）**：新增 **AI霖子 Skill Studio**：5 个无脚本官方模板、结构化访谈创建、权限/版本/全文预览、创建后试运行、ZIP 导入导出；Skill 文件夹原子写入，失败不留半成品。Vault 文件任务支持一个变更集中最多 12 篇 Markdown，逐篇完整预览与版本锁定，中途失败自动回滚；关键信息不足时可用原生问答卡继续同一任务。新能力全部按 ≥0.7.67 版本门禁；当前正式版 0.7.66 的服务端语义保持不变，本候选版未发布、未部署、未替换现有用户安装。
