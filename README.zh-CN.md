...
- **下一版本（待发布）**：小红书图文卡片会自动生成 3 个可选爆款标题、300–800 字正文和 5–8 个话题词；公众号原文内全部可读取配图按顺序进入卡片，除首图用于首页外，其余图片都与上下文文字混排，不再一张图独占一页。
...
- 第一张配图完整显示且不加图片外框；其余 Markdown、Wiki 或 HTML 图片按原文顺序进入正文卡片，并与图片前后的文字贪心分页、尽量填满每一页，不把图片单独做成一页。第一页已经显示的正文会从后续分页中扣除，Markdown 金句加粗继续保留。
