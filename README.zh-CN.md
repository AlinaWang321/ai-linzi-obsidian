...
### 小红书 3:4 图文分发（v0.6.44）

- 从公众号文章运行「多平台分发」，云端生成小红书配文，插件在用户电脑本机用公众号全文生成 3:4 图片，不上传卡片文件。
- 第一页仅在源文章 frontmatter 明确存在 `摘要`、`一句话摘要`、`summary`、`description` 或 `digest` 时显示摘要；没有摘要就不补正文第一段。
- 源文章有配图时，封面完整显示第一张配图，图片在上、标题和摘要在下，不加图片外框；正文卡片自动分页并保留 Markdown 金句加粗。
- 输出包括逐页 PNG、`小红书卡片.zip` 和一篇小红书笔记。笔记结构固定为图片在前、配文在后，不复制公众号全文。
- 单独运行「小红书卡片：当前笔记 → 3:4 PNG（本地生成）」不会调用 AI；配文优先读取源文章 frontmatter 的 `小红书配文`、`小红书文案`、`小红书正文` 或 `xhs_caption`，没有时仅回退到明确摘要。
